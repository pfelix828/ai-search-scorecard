      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-3">
        ...
        <Card>
          ...
          <div className="space-y-3 text-xs leading-relaxed text-muted">
            <p>
              <strong className="font-semibold text-foreground">Visibility Score</strong> is a 0-100 composite of four
              signals: how often the product is mentioned in sampled answers (mention rate, weighted{" "}
              {pct(w.mentionRate)}), how early in the answer the first mention lands (position, {pct(w.position)}), what
              share of the cited sources point to adobe.com (citation share, {pct(w.citationShare)}), and how positive the
              framing is when the product is mentioned (sentiment, {pct(w.sentiment)}). The weights are illustrative
              choices for this demo, not a measured truth.
            </p>
            <p>
              Headline scores are <strong className="font-semibold text-foreground">blended</strong>, meaning
              usage-weighted: each engine counts in proportion to its assumed share of AI-assistant usage, so a move on ChatGPT shifts the
              blended score more than the same move on Copilot.
            </p>
            <p>
              <strong className="font-semibold text-foreground">Weekly reads are noisy.</strong> Each number comes from
              a limited number of prompt runs ({meta.runsPerPromptEngineWeek} per prompt, engine, and week in this
              simulation), so every read carries a{" "}
              <Term def="The range a metric would plausibly fall in if we re-ran the same prompts again. Moves that stay inside this range are consistent with random variation.">
                confidence interval
              </Term>
              . A single-week move inside that interval is not news. Look for multi-week shifts, or moves tied to a
              known change like a content update or an engine model update.
            </p>
            <p>
              <strong className="font-semibold text-foreground">Targets are commitments, not decoration.</strong>{" "}
              Each product carries a Q3 FY26 Visibility Score target. A miss that sampling noise can explain is
              reported as exactly that; a real miss is reported as a real miss, with the diagnosis next to it.
            </p>
          </div>
        </Card>
      </div>
