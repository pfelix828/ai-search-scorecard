...
// Three pages, in the order a first-time reader should take them:
// the story, the live proof, the rigor. Nothing else is in the path.
const items = [
  { href: "/", label: "Summary", match: (p: string) => p === "/" },
  { href: "/overview", label: "Scorecard", match: (p: string) => p.startsWith("/overview") },
  { href: "/methodology", label: "How it works", match: (p: string) => p.startsWith("/methodology") },
];
